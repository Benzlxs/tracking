...
class Sort_3d(object):
    ...
    def update_range(self, object_dets, robot_loc, reset_confid=True, unknow_object_label=256):
        """
        Args:
            object_dets:  a numpy array of detections in the format
            [class, x, y, z, l, w, h, theta]
        ...
        for t,trk in enumerate(self.trackers):
          if(t not in unmatched_trks):
            ...
            if _distance < self.interest_range and int(trk.category)==unknow_object_label:
        ...
        for i in unmatched_dets:
            ...
            if _distance > self.interest_range:
                object_dets[i,0] = unknow_object_label
            else:
                num_classification_run += 1
