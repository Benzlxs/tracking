...
@numba.njit
def dist_points_to_points_jit_bk(points_1, points_2, h_dist, min_connect_points):
    """
...
def dist_points_to_points_jit(points_1, points_2, h_dist, min_connect_points):
    ...
    """
    points_2 = np.expand_dims(points_2, axis=1)
    _len_0 = points_1.shape[1]
    _len_1 = points_2.shape[0]
    all_dist = (points_1[:,0:_len_0:3] - points_2[0:_len_1:3,:])**2 +\
               (points_1[:,1:_len_0:3] - points_2[1:_len_1:3,:])**2 +\
               (points_1[:,2:_len_0:3] - points_2[2:_len_1:3,:])**2
    all_dist = all_dist + small_number
    all_dist = np.sqrt(all_dist)
    _ind_con = np.where(all_dist < h_dist)
    if _ind_con[0].shape[0] > min_connect_points:
...
def find_segs_one_scan(points, H_dist, min_seg_point):
    ...
    for x in range(num_segs):
        point_array = points[segs[x, 0]:(segs[x, 1] + 1), :3].astype(np.float32).reshape(-1)
        #point_array = point_array.reshape(-1)
        seg_list.append( point_array)
        label_list.append(x)
    # seg_list = [points[segs[x, 0]:(segs[x, 1] + 1), :3] for x in range(num_segs)]
    # label_list = [x for x in range(num_segs)]
    return seg_list, label_list, num_segs


@numba.njit
def find_connection(one_seg, all_segs_list, all_label_list, h_dist=0.5, min_connect_points=2):
    """
    ...
    for i in range(len(all_label_list)):
        ...
        FLAG = dist_points_to_points_jit(points_1, all_segs_list[i], h_dist,
                                         min_connect_points)
        if FLAG:
            # _label = all_label_list[i]
            connects.append(all_label_list[i])

    return connects


@numba.njit
def cluster_main_process(points, angle_dif, min_scan_points, min_seg_point, min_connect_points, V_dist, H_dist ):
    ...
    above_segs_list, above_label_list, num_segs = find_segs_one_scan(
        points[start_point:end_point, :], V_dist, min_seg_point)
    global_label = num_segs - 1
    global_segs_list = above_segs_list.copy()
    global_label_list = above_label_list.copy()
    for i in range(1, num_scans):
        start_point = all_scans[i, 0]
        end_point = all_scans[i, 1] + 1
        current_segs_list, current_label_list, num_segs = find_segs_one_scan(
            points[start_point:end_point, :], V_dist, min_seg_point)

        _temp_seg_list = [np.array([0.0,0.0,0.0],dtype=np.float32)]
        _temp_label_list = [1033]
        for j in range(num_segs):
            # the empty list should not be passed into nonpython function, for
            # numba usage
            if len(above_label_list)>0:
                connect_index = find_connection(current_segs_list[j], above_segs_list, above_label_list,
            ...
            if num_connect == 0:
                ...
                global_segs_list += [current_segs_list[j]]
                global_label_list +=[global_label]
                _temp_seg_list +=[current_segs_list[j]]
                _temp_label_list +=[global_label]
                #_temp_dict[global_label] = current_segs[j]
            # only one connection, merge it into global segs and check _temp_dict
            if num_connect == 1:
                _loc_global_ = global_label_list.index(connect_index[0])
                global_segs_list[_loc_global_] = np.hstack(
                    (global_segs_list[_loc_global_], current_segs_list[j]))
                ...
                if connect_index[0] in _temp_label_list:
                    ...
                    _temp_seg_list[_loc_local_] = np.hstack(
                        (_temp_seg_list[_loc_local_], current_segs_list[j]))
                else:
                    # adding new segment
                    _temp_seg_list += [current_segs_list[j]]
                    _temp_label_list +=[connect_index[0]]
            ...
            if num_connect >= 2:
                ...
                for m in connect_index:
                    ...
                    global_segs_list[_loc_global_1] = np.hstack(
                        (global_segs_list[_loc_global_1], global_segs_list[_loc_global_2]))
                    # delete globally
                    global_segs_list.pop(_loc_global_2)
                    global_label_list.pop(_loc_global_2)

                    # merge locally
                    _loc_above_1 = above_label_list.index(min_label)
                    _loc_above_2 = above_label_list.index(m)
                    above_segs_list[_loc_above_1] = np.hstack(
                        (above_segs_list[_loc_above_1], above_segs_list[_loc_above_2]))
                    # delete locally
                    above_segs_list.pop(_loc_above_2)
                    above_label_list.pop(_loc_above_2)


                    if m in _temp_label_list:
                        _loc_local_1 = _temp_label_list.index(min_label)
                        _loc_local_2 = _temp_label_list.index(m)
                        _temp_seg_list[_loc_local_1] = np.hstack(
                            (_temp_seg_list[_loc_local_1], _temp_seg_list[_loc_local_2]))
                        # delete in temporaral vector
                        _temp_seg_list.pop(_loc_local_2)
                        _temp_label_list.pop(_loc_local_2)
                        # _temp_dict.pop(m)
        _temp_seg_list.pop(0)
        _temp_label_list.pop(0)
        above_segs_list = _temp_seg_list.copy()
        above_label_list = _temp_label_list.copy()

    return global_segs_list, global_label_list
...
class PointCluster(object):
    ...
    def scan_run_(self, points):
        ...
        """
        assert points.shape[1]==3,"point size should be N by 3"
        ...
        global_segs_list, global_label_list = cluster_main_process(points, angle_dif, min_scan_points, min_seg_point, min_connect_points, V_dist, H_dist
        )
        ...
        for i in range(len(global_label_list)):
            global_segs_list[i] = global_segs_list[i].reshape(-1,3)

        return global_segs_list, len(global_label_list)
