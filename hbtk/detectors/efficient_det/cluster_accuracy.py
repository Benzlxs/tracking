...
import numpy as np
from utils import kitti_common as kitti
...
from utils.eval import get_recall_result, get_official_eval_result
...
class ClusterEvaluation(object):
    ...
    def convert_detection_to_kitti_annos(self, detection):
        ...
        for i in range(len(detection)):
            ...
            P2 = calib["P2"]
            pred_cls_names = det["label_preds"]
            ...
            for j in range(box3d_lidar.shape[0]):
                ...
                anno["name"].append(pred_cls_names[j])
    ...
    def evaluation_recall(self, detections, output_dir):
        """
        the z axis and box z center.
        """
        if self._tiny_data is None:
            gt_annos = self.ground_truth_annotations
        else:
            gt_annos = self.ground_truth_annotations[self._tiny_data[0]:self._tiny_data[1]]
        ...
        return {
            ...
            "num_tp_fp_fn": result_dict["num_tp_fp_fn"],
        }

    def evaluation_accuracy(self, detections, gt_annos, iou=0.7, output_dir='./results'):
        """
        the z axis and box z center.
        """
        #if self._tiny_data is None:
        ...
        z_center = 1.0  # KITTI camera box's center is [0.5, 1, 0.5]
        # for regular raw lidar data, z_axis = 2, z_center = 0.5.
        result_dict = get_official_eval_result(
                            gt_annos,
                            dt_annos,
                            self._class_names,
                            iou = iou,
    ...
    def save_det_results(self, detection, result_save_path = './results/det'):
        class_names = self._class_names
        ...
        for i in range(len(detection)):
            ...
            for j in range(box3d_lidar.shape[0]):
                ...
                result_dict ={
                    'name':pred_cls_names[j],
                    'alpha':-np.arctan2(-box3d_lidar[j, 1], box3d_lidar[j, 0])+box3d_camera[j, 6],
                    'bbox':bbox[j],
                    'location':box3d_camera[j, :3],
                    'dimensions':box3d_camera[j, 3:6],
                    'rotation_y':box3d_camera[j, 6],
                    'score':scores[j],
                }
            ...
            result_file = f"{result_save_path}/{kitti.get_image_index_str(det_idx)}.txt"
            result_str = '\n'.join(result_lines)
...
def _create_reduced_point_cloud(data_path,
    ...
    for info in prog_bar(kitti_infos):
        ...
        img_path = Path(data_path) / image_info['image_path']
        ...
        points_v = box_np_ops.add_rgb_to_points(points_v, img, rect, Trv2c, P2)
