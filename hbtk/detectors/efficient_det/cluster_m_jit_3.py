...
def find_segs_one_scan(points, H_dist, min_seg_point):
    ...
    for x in range(num_segs):
        seg_list += [points[segs[x, 0]:(segs[x, 1] + 1), :3]]
        label_list += [x]
    ...
    def scan_run_(self, points):
        ...
        for i in range(1, num_scans):
            ...
            for j in range(num_segs):
                connect_index = find_connection(current_segs[j], above_segs_list, above_label_list,
                ...
                if num_connect == 1:
                    ...
                    global_segs_list[_loc_global_] = np.vstack(
                        (global_segs[_loc_global_], current_segs_list[j]))
                    ...
                    if connect_index[0] in _temp_label_list:
                        ...
                        _temp_seg_list[_loc_local_] = np.vstack(
                            (_temp_seg_list[_loc_local_], current_segs_list[j]))
                    else:
                        # adding new segment
                        _temp_seg_list += current_segs[j]
                ...
                if num_connect >= 2:
                    ...
                    if min_label in _temp_label_list:
                        ...
                        _temp_seg_list[_loc_local_] = np.vstack(
                            (_temp_seg_list[_loc_local_], current_segs[j]))
                    else:
                        _temp_seg_list += [current_segs[j]]
                    ...
                    global_segs_list[_loc_global_] = np.vstack(
                        (global_segs[_loc_global_], current_segs[j]))
                    ...
                    for m in connect_index:
                        ...
                        global_segs_list[_loc_global_1] = np.vstack(
                        ...
                        above_segs_list[_loc_above_1] = np.vstack(
                        ...
                        if m in _temp_label_list:
                            ...
                            _temp_seg_list[_loc_local_1] = np.vstack(
