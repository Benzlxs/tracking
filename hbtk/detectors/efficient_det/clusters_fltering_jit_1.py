...
#@numba.njit
def calculate_ang_overlap_jit(lab_ang):
    """
    calculate the angle overlap matrix
            ang_overlap = np.max(0, np.min(rightmost1, rightmost2 ) - np.max(leftmost1, leftmost2));
            construct the matrix using above equations
    ...
    lab_ang = np.array(lab_ang)
    #h_ang_left, v_ang_left = np.meshgrid(lab_ang[:, 0], lab_ang[:, 0])
    h_ang_left, v_ang_left = numba_meshgrid(lab_ang[:, 0])
    h_ang_left = np.expand_dims(h_ang_left, axis=2)
    v_ang_left = np.expand_dims(v_ang_left, axis=2)

    # h_ang_right, v_ang_right = np.meshgrid(lab_ang[:, 1], lab_ang[:, 1])
    h_ang_right, v_ang_right = numba_meshgrid(lab_ang[:, 1])
    h_ang_right = np.expand_dims(h_ang_right, axis=2)
    v_ang_right = np.expand_dims(v_ang_right, axis=2)
...
@numba.njit
def numba_meshgrid(a):
    xx = a.shape[0]
    h_a = np.zeros((xx,xx),dtype=a.dtype)
    v_a = np.zeros((xx,xx),dtype=a.dtype)
    for i in range(xx):
        h_a[:,i] = a[i]
        v_a[i,:] = a[i]
    return h_a, v_a
...
def calculate_range_overlap(lab_rng):
    ...
    h_rng, v_rng = numba_meshgrid(lab_rng)
    ...
    rng_overlap_matrix = np.max(diff_rng_zeros, axis=2)
...
def filtering_with_xyz_jit(clusters, max_length_cluster, max_width_cluster):
    ...
    for _c in mm_xyz:
        ...
        y_min = _c[2]
        y_max = _c[3]
        if ((x_max - x_min) < max_length_cluster) and \
...
#@numba.njit
def get_overlap_max_jit(h_x_min, v_x_min, h_x_max, v_x_max, h_y_min, v_y_min, h_y_max, v_y_max):
    """
    calculate the overlap matrix
...
@numba.njit
def filtering_with_num_points_jit(clusters, frustum_max_x, frustum_offset, frustum_ratio, angle_threshold):
    """
    ...
    for c in clusters:
        ...
        if _x_mean < frustum_max_x:
            all_ang = find_all_angle(c)
            leftmost_ang = np.min(all_ang) - angle_threshold
            rightmost_ang = np.max(all_ang) + angle_threshold
    ...
    # lab_ang = np.array(lab_ang)
    # lab_rng = np.array(lab_rng)
    ang_overlap_matrix = calculate_ang_overlap_jit(lab_ang)
    ...
    for i in range(len(lab_clusters)):
        if occlusion_matrix[i] == 0:  # occluson level = 0
            # mean_rng = np.mean(np.sqrt(lab_clusters[i][:,0]**2 + lab_clusters[i][:,1]**2 + lab_clusters[i][:,2]**2))
            _x_mean = np.mean(lab_clusters[i][:, 0])
            _y_mean = np.mean(lab_clusters[i][:, 1])
            # located in region of interest
            if (_y_mean < frustum_ratio*(_x_mean - frustum_offset)) and \
                    (-_y_mean < frustum_ratio*(_x_mean - frustum_offset)):
                mean_rng = np.mean(find_all_range(lab_clusters[i]))
                min_num_points = check_min_num_points(mean_rng)
                if lab_clusters[i].shape[0] > min_num_points:
...
def merge_clusters_jit(clusters, mm_xyz):
    """
    merge two clusters
    """
    # mm_xyz = np.array(mm_xyz)
    s_mm_xyz = np.array(mm_xyz)  # n*6
    h_x_min, v_x_min = numba_meshgrid(s_mm_xyz[:, 0])  # n*n*1
    h_x_min = np.expand_dims(h_x_min, axis=2)
    v_x_min = np.expand_dims(v_x_min, axis=2)

    h_x_max, v_x_max = numba_meshgrid( s_mm_xyz[:, 1])
    h_x_max = np.expand_dims(h_x_max, axis=2)
    v_x_max = np.expand_dims(v_x_max, axis=2)

    h_y_min, v_y_min = numba_meshgrid( s_mm_xyz[:, 2])
    h_y_min = np.expand_dims(h_y_min, axis=2)
    v_y_min = np.expand_dims(v_y_min, axis=2)

    h_y_max, v_y_max = np.meshgrid( s_mm_xyz[:, 3])
    h_y_max = np.expand_dims(h_y_max, axis=2)
    v_y_max = np.expand_dims(v_y_max, axis=2)

    overlap_matrix = get_overlap_max_jit(h_x_min, v_x_min, h_x_max, v_x_max, h_y_min, v_y_min, h_y_max, v_y_max)
...
class ClustersFiltering(object):
    ...
    def __init__(self, config):
        ...
        clustersfiltering_dict = {
            "filtering_with_all": self.filtering_with_all,
        }
        self._clusterfiltering_ = clustersfiltering_dict[
            self.clusterfiltering_method]

    def filtering_with_all(self, clusters):
        """
        ...
        #s_clusters = clusters.copy()
        # remove extreme large size
        # s_clusters, mm_xyz = self.filtering_with_xy(s_clusters, mm_xyz)
        s_clusters, mm_xyz = filtering_with_xyz_jit(clusters, self.max_length_cluster, self.max_width_cluster)

        # merge the clusters with overlap in vertical direction
        s_clusters = self.merge_clusters(s_clusters, mm_xyz)
        # s_clusters = merge_clusters_jit(s_clusters, mm_xyz)

        # remove clusters with small hight
        s_clusters = self.filtering_with_z(s_clusters)
        # s_clusters = filtering_with_z_jit(s_clusters, self.max_hight_cluster)

        # remove clusters with too small number of points
        s_clusters = self.filtering_with_num_points(s_clusters)
        # s_clusters = filtering_with_num_points_jit(clusters, self.frustum_max_x, self.frustum_offset, self.frustum_ratio, self.angle_threshold)
